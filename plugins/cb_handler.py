...
async def cb_handler(c: Client, cb: CallbackQuery):
    ...
    if cb.data == "merge":
        ...
        UPLOAD_AS_DOC.update({f"{cb.from_user.id}": True})
        await cb.message.edit(
            text="Do you want to rename? Default file name is **DEFAULT_softmuxed_video.mkv**",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
        ...
        UPLOAD_AS_DOC.update({f"{cb.from_user.id}": False})
        await cb.message.edit(
            text=f"Do you want to rename? Default file name is **DEFAULT_softmuxed_video.mkv**",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("👆 Default", callback_data="renameS_NO"),
        ...
        if "YES" in cb.data:
            await cb.message.edit(
                "Current filename: **DEFAULT_softmuxed_video.mkv**\n\nSend me new file name without extension: You have 1 minute"
            )
            res: Message = await c.listen(
        ...
        if "NO" in cb.data:
            await mergeSub(
                c,
                cb,
                new_file_name=f"./downloads/{str(cb.from_user.id)}/DEFAULT_softmuxed_video.mkv",
            )
